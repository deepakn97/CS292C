...
\begin{enumerate}
    \renewcommand{\labelenumi}{\arabic{enumi}.}
    \item Ensure that the output is ordered i.e. for every i, j belonging to $[0, a.Length]$, if i <= j then $a[i] <= a[j]$.
    \item Ensure that the multiset of the output is the same as the input i.e. all elements of the array are preserved.
\end{enumerate}

\part
\textbf{Outer Loop Invariants}
\begin{enumerate}
    \renewcommand{\labelenumi}{\arabic{enumi}.}
    \item Ensure that the current index is within range.
    \item Ensure that the subarray from $a[i:a.Length]$ is ordered.
    \item Ensure that the right side parition contains all elements greater than the left side. This is to ensure that the larger numbers are infact bubbling towards the end of the array.
    \item Ensure that the array elements are preserved.
\end{enumerate}

\part
\textbf{Inner Loop Invariants}
\begin{enumerate}
    \renewcommand{\labelenumi}{\arabic{enumi}.}
    \item Ensure that the subarray from $a[i:a.Length]$ is ordered.
    \item Ensure that the right side parition contains all elements greater than the left side. This is to ensure that the larger numbers are infact bubbling towards the end of the array.
    \item Ensure that the inner loop is bubbling the maximum element, i.e. the element at current inner loop index should be bigger than all the elements to it's left. 
    \item Ensure that the array elements are preserved.
\end{enumerate}
