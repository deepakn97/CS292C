...
(** The [apply] tactic also works with _conditional_ hypotheses
    and lemmas: ifthe statement being applied is an implication, then
...
(* 
  The [apply] tactic is used to match the current goal with the conclusion of a previously proven theorem or hypothesis, effectively replacing the goal with the premises of that theorem or hypothesis. The [rewrite] tactic, on the other hand, is used to replace a term in the goal using an equality or equivalence relation. Both tactics can be useful when you need to transform the goal to match a known fact: [apply] is useful when the goal matches the conclusion of a theorem, while [rewrite] is useful when you need to substitute terms based on known equalities.
...
Proof.
  intros n m o p eq1 eq2.
  transitivity (n + p). reflexivity. rewrite eq1 in eq2. apply eq2.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite eq2 in eq1. injection eq1.
  intros H0 H1.
  transitivity z. apply H1. symmetry. apply H0.
Qed.
...
Proof.
  ...
  - intros m eq. destruct m as [|m'] eqn:E.
    + simpl in eq. discriminate eq.
    + simpl in eq. injection eq as goal. rewrite <- plus_n_Sm in goal. rewrite <- plus_n_Sm in goal.
      injection goal as goal1. apply Ihn in goal1. f_equal. apply goal1.
Qed.
...
Proof.
  ...
  - destruct (f true) eqn:H1.
    + rewrite H1. destruct (f true) eqn:H2.
      * reflexivity.
      * discriminate H1.
...
Proof.
  intros n.
  induction n as [|n' Ihn].
  - intros m. destruct m as [|m'] eqn:Eqm.
    + reflexivity.
    + simpl. reflexivity.
  - intros m. destruct m as [|m'] eqn:Eqm.
    + simpl. reflexivity.
...
Proof.
  intros n m p.
  transitivity (m =? p).
  - apply eqb_true in H. rewrite H. reflexivity.
  - apply H0.
Qed.
(** [] *)
