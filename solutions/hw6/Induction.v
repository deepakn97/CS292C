...
Theorem add_0_r_left_side: forall n:nat,
  0 + n = n.
Proof.
  intros n.
  reflexivity.
...
Proof.
  ...
  - simpl. rewrite <- plus_n_Sm. f_equal. apply IHn'.
Qed. 
...
Proof.
  ...
  induction n as [|n'].
  - simpl. reflexivity.
  - simpl. f_equal. rewrite IHn'. rewrite plus_n_Sm. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl.
    rewrite plus_eq_S with (n := bin_to_nat b0' + (bin_to_nat b0' + 0)).
    reflexivity.
  - simpl. 
    rewrite IHb1'. simpl. f_equal.
    rewrite add_0_r. rewrite plus_eq_S. rewrite add_assoc. reflexivity.
Qed.
...
(* There can be countless zeros before any binary number. So B1(B1 Z) cal also be written as 
B1(B1 (B0 Z)). So in normalize function we will remove all the zeroes I guess.*)
...
Example normalize_3_two_0s : normalize (B1 (B1 (B0 (B0 Z)))) = B1 (B1 Z).
Proof. simpl. reflexivity. Qed.
...
Lemma nat_to_bin_add_1 : forall n:nat,
...
Proof.
  ...
  induction b as [|b0'|b1'].
  - simpl. reflexivity.
  - simpl. rewrite add_0_r. rewrite <- IHb0'. rewrite double_nat_to_bin. reflexivity.
  - simpl. rewrite add_0_r. rewrite <- IHb1'. 
    rewrite <- double_nat_to_bin with (n := bin_to_nat b1').
    rewrite nat_to_bin_add_1. f_equal.
Qed.
