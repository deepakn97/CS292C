...
(** We could try to proceed by case analysis or induction on [n].  But
  ...
  - (* E = ev_0 *) unfold Even. exists 0. reflexivity.
  - unfold Even. (* E = ev_SS n' E' 
...
Proof.
  ...
  induction E as [| n' E' IH].
  - intros m H. simpl. apply H.
  - intros m H. simpl. apply ev_SS. apply IH. apply H.
Qed.
...
Proof.
  ...
  rewrite <- add_assoc in H. rewrite (add_assoc m n p) in H.
  rewrite (add_comm m n) in H. rewrite add_assoc in H. rewrite add_assoc in H.
...
Proof.
  ...
  - destruct IH as [H| H].
    ...
    + unfold ge in H. inversion H.
      * left. reflexivity.
      * right. unfold ge. apply n_le_m__Sn_le_Sm in H0. apply H0.
Qed.
...
Proof.
  ...
  - intros. destruct p.
    ...
    + simpl in H. rewrite plus_n_Sm with n' m in H. rewrite plus_n_Sm with p q in H.
...
Proof.
  ...
  - intros. simpl. apply n_le_m__Sn_le_Sm. apply IH. apply H.
Qed.
...
Proof.
  induction n as [|n' IH].
  - intros. apply O_le_n.
  - intros. unfold lt in H. apply le_S in H. apply Sn_le_Sm__n_le_m in H. apply H.
Qed.
...
Proof.
  ...
  - intros. destruct m.
    + discriminate.
    + apply IH in H. apply n_le_m__Sn_le_Sm in H. apply H.
Qed.
...
Proof.
  ...
  - destruct n.
    + reflexivity.
    + intros. apply Sn_le_Sm__n_le_m in H. apply (IH n H).
Qed.
...
.
Check plus.
Theorem example_1: R 1 1 2.
Proof.
  apply c2. apply c3. apply c1.
Qed.

Theorem example_2: R 2 1 3.
Proof.
  apply c2. apply c2. apply c3. apply c1.
Qed.
...
(** - Which of the following propositions are provable?
    ...
    - If we dropped constructor [c5] from the definition of [R],
      ...
      No. Since we can prove all the provable propositions using c1 c2 c3.
...
Inductive R2 : nat -> list nat -> Prop :=
  | c1                    : R2 0     []
  | c2 n l (H: R2 n     l) : R2 (S n) (n :: l)
  | c3 n l (H: R2 (S n) l) : R2 n     l.

Theorem example_R2_1: R2 2 [1; 0].
Proof.
  apply c2. apply c2. apply c1.
Qed.

Theorem example_R2_2: R2 1 [1;2;1;0].
Proof.
  apply c3. apply c2. apply c3. apply c3. apply c2. apply c2. apply c2.
  apply c1.
Qed.

Theorem example_R3_3: R2 6 [3;2;1;0].
Proof.
Abort.
...
Proof.
  intros P b H. destruct H as [HP | HnP].
  - split. 
    + reflexivity.
    + intros. apply HP.
  - split.
    + intros HP. exfalso. apply HnP. apply HP.
...
Proof.
  ...
  - simpl in Hcount. destruct (eqbP n m) as [H | H].
    + inversion Hcount.
    + intros H1. destruct H1 as [Heq | HIn].
      * symmetry in Heq. apply H. apply Heq.
      * apply IHl'. apply Hcount. apply HIn.
Qed.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | ns0: nostutter []
  | ns1 x : nostutter [x]
  | ns2 x y l (P: x <> y) (H: nostutter (y :: l)) : nostutter (x :: y :: l)
.
...
Proof.
  intro H.
  inversion H as [| | x1 y1 l1 H1 H2].
  inversion H2 as [| | x2 y2 l2 H5 H6].
...
| pal_nil : pal []
| pal_single x : pal [x]
| pal_add x l (H: pal l) : pal (x :: l ++ [x]).

Theorem pal_example1: pal [1].
Proof.
  apply pal_single.
Qed.

Theorem pal_example2: pal (@nil nat).
Proof.
  apply pal_nil.
Qed.

Theorem pal_example3: pal [1; 2; 1].
Proof.
  (* why can't we apply pal_add directly here. *)
  apply (pal_add 1 [2]).
  apply pal_single.
Qed.
...
Proof.
  ...
  - simpl. apply pal_nil.
  - simpl. rewrite app_assoc. apply pal_add. apply IH.
Qed.

Theorem pal_rev : forall (X:Type) (l: list X) , pal l -> l = rev l.
Proof.
  intros X l H.
  induction H as [| |x l H IH].
  - reflexivity.
  - reflexivity.
