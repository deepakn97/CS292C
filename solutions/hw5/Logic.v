...
Proof.
  ...
  - simpl in H. apply and_exercise in H. 
    destruct H as [Hm _]. right. apply Hm.
Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q. intros H. unfold not. intros HQ.
  intros. apply H in H0. apply HQ. apply H0.
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros. unfold not. intros H. destruct H. apply H0. apply H.
Qed.
(** [] *)
...
Proof.
  intros X P H.
  intros [x Hx]. unfold not in Hx. apply Hx. apply H.
Qed.
(** [] *)
...
Proof.
  intros n.
  induction n as [|n' Ihn].
  - intros m [x Hx]. simpl. reflexivity.
  - intros m [x Hx]. destruct m as [|m'].
    + simpl in Hx. discriminate Hx.
    + simpl. apply Ihn. exists x. injection Hx as HI. apply HI.
Qed.
...
Proof.
  ...
  - intros H.
    ...
    + simpl in H. destruct H as [H | H].
      ...
      * apply Ihl in H. destruct H as [a [H1 H2]]. exists a. split.
...
Proof.
  ...
  - intros H. induction l as [|x l' IH].
    + simpl. intros x Hx. destruct Hx.
    + simpl. intros y Hy. destruct Hy as [Hy | Hy].
      * rewrite <- Hy. apply H.
      * apply IH. apply H. apply Hy.
Qed.
...
Proof.
  ...
  - destruct IH as [k IHk].
    rewrite even_S. destruct (even n') eqn:En'.
    + simpl. exists k. rewrite IHk. reflexivity.
    + exists (S k). simpl. rewrite IHk. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros b1 b2. split.
  - intros H. destruct b1, b2; split; try reflexivity; 
    simpl in H; discriminate H.
  - intros [H1 H2].
...
Proof.
  ...
  - intros H. apply not_true_iff_false in H. 
    unfold not in H. rewrite eqb_eq in H. unfold not. apply H.
  - intros H. apply not_true_iff_false. unfold not. rewrite eqb_eq.
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
  ...
  match l1, l2 with
  | [], [] => true
  | x1::t1, x2::t2 => eqb x1 x2 && (eqb_list eqb t1 t2)
  | _, _ => false
  end.
...
Proof.
  ...
  - generalize dependent l2.
    induction l1 as [|x1 t1 IH]; intros l2.
    + destruct l2 as [|x2 t2].
      * reflexivity.
      * simpl. intros H1. discriminate H1.
    + destruct l2 as [|x2 t2].
      * simpl. intros H1. discriminate H1.
      * simpl. intros H1. apply andb_true_iff in H1.
        destruct H1 as [H2 H3].
        apply H in H2. rewrite H2.
        apply IH in H3. rewrite H3.
        reflexivity.
  - intros H1. subst.
    induction l2 as [|x2 t2 IH].
    + reflexivity.
...
Lemma rev_append_correct : forall (A: Type) (l1 l2 : list A),
...
Proof.
  ...
  apply H. right. intros p. apply H. left. apply p.
Qed.
...
Theorem dist_not_exists_part2 : forall (X:Type) (P : X -> Prop),
  (forall x, P x) -> ~ (exists x, ~ P x).
Proof.
  intros X P H.
  intros [x Hx]. unfold not in Hx. apply Hx. apply H.
