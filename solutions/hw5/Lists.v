...
Fixpoint isoddnumber (n: nat) : bool :=
  match n with
  | 0 => false
  | S n' => negb (isoddnumber n')
  end.
...
Fixpoint oddmembers (l:natlist) : natlist := 
  ...
  | h::t => match h with
    | 0 => oddmembers t
    | _ => if isoddnumber h then h :: (oddmembers t) else oddmembers t
    end
  end.
...
Proof.
  intros.
  rewrite H. rewrite <- H0. rewrite <- H. reflexivity.
Qed.
